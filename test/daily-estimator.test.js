...
test('estimateDaily: sessioni da Drive+canali nel prompt, storico correzioni', async function() {
  var prompt;
  var fakeClient = { messages: { create: async function(req) { prompt = req.messages[0].content; return { content: [{ type: 'text', text: JSON.stringify({ oggi: [{ task: 'Landing Elios', hours: 2, minutes: 0 }], domani: [], blocchi: null, confidence: 'alta', note: '' }) }] }; } } };
  var fakeDb = { getLogsForUserDate: async function() { return []; }, getProject: async function() { return null; } };
  var dayContext = {
    ...
    driveByEmail: {}, driveByName: {}, driveEvents: { byEmail: { 'samuele@k.it': [{ at: '2026-09-10T09:05:00+02:00', kind: 'drive', name: 'Copy landing' }, { at: '2026-09-10T09:30:00+02:00', kind: 'drive', name: 'Landing Elios' }, { at: '2026-09-10T10:45:00+02:00', kind: 'drive', name: 'Landing Elios' }, { at: '2026-09-10T15:00:00+02:00', kind: 'drive', name: 'Landing Elios' }] }, byName: {} },
    adminEvents: [],
  };
  ...
  assert.match(prompt, /SESSIONI DI LAVORO RICOSTRUITE DAI TIMESTAMP \(totale 1h55min\):\n- 09:05–10:45 \(1h40min\): Drive "Copy landing" \(1 modifiche\); Drive "Landing Elios" \(2 modifiche\); #elios \(1 messaggi\)\n- 15:00–15:00 \(15min\): Drive "Landing Elios" \(1 modifiche\)/);
  assert.match(prompt, /STORICO DELLE STIME PER QUESTA PERSONA: negli ultimi 5 daily corretti/);
  assert.deepEqual(out.estimate.sources, ['messaggi nei canali', 'sessioni di lavoro']);
...
test('review Codex: bucket email e nome uniti senza doppioni; revisioni confrontate come istanti e scansione con budget di tempo', async function() {
  ...
  var fakeDb = { getLogsForUserDate: async function() { return []; }, getProject: async function() { return null; } };
  var same = { at: '2026-09-10T09:30:00+02:00', kind: 'drive', name: 'Landing' };
  var dayContext = { users: [{ id: 'U1', name: 'Samuele Licciardello', email: 'samuele@k.it' }], slackByUser: {},
    driveByEmail: { 'samuele@k.it': [{ name: 'Landing', type: 'document', modified_at: 'a' }] }, driveByName: { 'samuele licciardello': [{ name: 'Landing', type: 'document', modified_at: 'a' }, { name: 'Icone', type: 'document', modified_at: 'b' }] },
    driveEvents: { byEmail: { 'samuele@k.it': [same] }, byName: { 'samuele licciardello': [same, { at: '2026-09-10T10:00:00+02:00', kind: 'drive', name: 'Icone' }] } }, adminEvents: [] };
...
test('registro posizioni: documenti e messaggi portano [progetto: X] nel prompt e nelle sessioni', async function() {
  var prompt;
...
test('calendario: riunioni senza cliente vanno alle attività trasversali, con il tag nel prompt e nelle sessioni', async function() {
  ...
  var fakeDb = { getLogsForUserDate: async function() { return []; }, getProject: async function() { return null; } };
  var dayContext = { users: [{ id: 'U1', name: 'Paolo', email: 'paolo@k.it' }], slackByUser: {}, driveByEmail: {}, driveByName: {}, driveEvents: { byEmail: {}, byName: {} },
