...
| Claude | C + B ("Modifico nel modulo" precompilato anche dal daily salvato; bottone "Scrivo a testo libero" con parser AI; stima con filtro sui blocchi di presenza e riunioni verificate con gli appunti Gemini) | `claude/clever-planck-8jyk6u` | 22/9 | pushato, in attesa di merge |
| Claude | C + F (registro ore riallineato ai daily: sync esportata, reconcileLedger notturno, proposta letta dal DB; diagnosi del secondo bot su Railway) | `claude/clever-planck-8jyk6u` (2ª PR) | 23/9 | pushato, in attesa di merge |
| Claude | G (giun.os: segnali con base dati, copertura dei daily, cache dei dati grezzi, sessione con cookie) | `claude/clever-planck-8jyk6u` (3ª PR) | 23/9 | pushato, in attesa di merge |
